import {get_three_days_wind, get_wind_data} from "../all/api/openweatherapi.js";
...
function draw()
{
    setTimeout(
        ()=>{
            get_three_days_wind("Minsk",get_wind_data,draw_polygon)
            draw_system()

        }, 1000);
}

function draw_polygon(winds)
{
...
function convert_winds_array(winds_array)
{
    ...
    {
        if (i[0] === "W" || i[0] === "S" || i[0] === "E" || i[0] === "N"){
            dict.set(i[0]+ " ", dict.get(i[0]+ " ") + i[1] * 5)
        }
        else{dict.set(i[0], dict.get(i[0]) + i[1] * 5)}
